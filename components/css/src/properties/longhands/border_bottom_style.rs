...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_border_bottom_style().clone(),
		initial_value(),
		LonghandId::BorderBottomStyle,
		PropertyDeclaration::BorderBottomStyle(value) => value.clone()
	);
	context.computed_values.set_border_bottom_style(computed_value);
}
