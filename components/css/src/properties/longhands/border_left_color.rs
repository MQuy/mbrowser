...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_border_left_color().clone(),
		initial_value(),
		LonghandId::BorderLeftColor,
		PropertyDeclaration::BorderLeftColor(value) => value.to_computed_value(context)
	);
	context.computed_values.set_border_left_color(computed_value);
}

/// https://drafts.csswg.org/css-backgrounds-3/#propdef-border-left-color
pub fn parse_declared<'i, 't>(
	_context: &ParserContext,
	input: &mut Parser<'i, 't>,
) -> Result<PropertyDeclaration, ParseError<'i>> {
	Color::parse(input).map(PropertyDeclaration::BorderLeftColor)
}
