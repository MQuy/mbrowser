...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_border_left_style().clone(),
		initial_value(),
		LonghandId::BorderLeftStyle,
		PropertyDeclaration::BorderLeftStyle(value) => value.clone()
	);
	context.computed_values.set_border_left_style(computed_value);
}

/// https://drafts.csswg.org/css-backgrounds-3/#propdef-border-left-style
pub fn parse_declared<'i, 't>(
	_context: &ParserContext,
	input: &mut Parser<'i, 't>,
) -> Result<PropertyDeclaration, ParseError<'i>> {
	LineStyle::parse(input).map(PropertyDeclaration::BorderLeftStyle)
}
