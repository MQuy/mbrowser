...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_border_left_width().clone(),
		initial_value(),
		LonghandId::BorderLeftWidth,
		PropertyDeclaration::BorderLeftWidth(value) => value.to_computed_value(context)
	);
	context.computed_values.set_border_left_width(computed_value);
}

/// https://drafts.csswg.org/css-backgrounds-3/#propdef-border-left-width
pub fn parse_declared<'i, 't>(
	_context: &ParserContext,
	input: &mut Parser<'i, 't>,
) -> Result<PropertyDeclaration, ParseError<'i>> {
	LineWidth::parse(input).map(PropertyDeclaration::BorderLeftWidth)
}
