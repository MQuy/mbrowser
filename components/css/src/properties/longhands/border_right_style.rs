...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_border_right_style().clone(),
		initial_value(),
		LonghandId::BorderRightStyle,
		PropertyDeclaration::BorderRightStyle(value) => value.clone()
	);
	context.computed_values.set_border_right_style(computed_value);
}
