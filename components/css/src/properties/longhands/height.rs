...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		declaration,
		context.parent_style.get_height().clone(),
		initial_value().to_computed_value(context),
		LonghandId::Height,
		PropertyDeclaration::Height(value) => value.to_computed_value(context)
	);
	context.computed_values.set_height(computed_value);
}
...
pub fn parse_declared<'i, 't>(
	...
	Size::parse(context, input).map(PropertyDeclaration::Height)
}
