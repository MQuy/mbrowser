...
pub fn cascade_property<'a>(declaration: Option<&PropertyDeclaration>, context: &'a mut StyleContext) {
	let computed_value = computed::from_non_inherited_property!(
		...
		PropertyDeclaration::PaddingLeft(value) => value.to_computed_value(context)
	);
	context.computed_values.set_padding_left(computed_value);
}
...
pub fn parse_declared<'i, 't>(
	...
	NonNegativeLengthPercentage::parse(input).map(PropertyDeclaration::PaddingLeft)
}
