...
pub fn load_cached_font(family_names: &Vec<String>) -> (&'static str, &'static [u8]) {
	static mut FONTCACHED: Option<HashMap<String, Vec<u8>>> = None;

	unsafe {
		if FONTCACHED.is_none() {
			FONTCACHED = Some(HashMap::new());
		}
		if let Some(fonts) = &mut FONTCACHED {
				...
				if let Some(_) = fonts.get(family_name) {
					...
					match handle {
						Ok(value) => {
							...
							fonts.insert(family_name.to_string(), bytes);
							matched_font_name = Some(family_name);
						},
						Err(_) => (),
					};
