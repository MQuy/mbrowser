...
fn default() {
	...
	assert_eq!(computed_values.get_border_right_color().clone(), RGBA::transparent());
}
...
fn from_author() {
	let tree = Rc::new(construct_tree(
		r#"<p id="test"></p>"#,
		r#"
#test { border-right-color: currentColor; color: red; }
        "#,
	));
	let dom = find_dom(&tree, "test").unwrap();
	let computed_values = GlobalScope::get_or_init_computed_values(dom.id());
	assert_eq!(
		computed_values.get_border_right_color().clone(),
