...
use css::values::computed::length::{LengthPercentage, MaxSize, Size};
...
fn default() {
	...
	assert_eq!(computed_values.get_max_width().clone(), MaxSize::None);
}
...
fn from_author() {
	let tree = Rc::new(construct_tree(
		r#"<p id="test"></p>"#,
		r#"
#test { max-width: 100px; }
        "#,
	));
	let dom = find_dom(&tree, "test").unwrap();
	let computed_values = GlobalScope::get_or_init_computed_values(dom.id());
	assert_eq!(
		computed_values.get_max_width().clone(),
